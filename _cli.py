...
from pip_audit._service import OsvService, PyPIService, VulnerabilityService, NvdService, PyUpService, GitHubAdvisoryService  # Add NvdService PyUpService GitHubAdvisoryService
...
class VulnerabilityServiceChoice(str, enum.Enum):
    ...
    Pyup = "pyup"
    Pypi = "pypi"
    Nvd = "nvd"  # Add NVD as a service choice
    Github = "github" # Add GitHub Advisories database as a service choice
    ...
    def to_service(self, timeout: int, cache_dir: Path | None) -> VulnerabilityService:
        ...
        elif self is VulnerabilityServiceChoice.Pyup:
            return PyUpService(cache_dir, timeout)
        elif self is VulnerabilityServiceChoice.Nvd:  # Handle NVD service choice
            return NvdService(cache_dir, timeout)
        elif self is VulnerabilityServiceChoice.Github:  # Handle Github service choice
            return GitHubAdvisoryService(cache_dir, timeout)
        else:
            assert_never(self)  # pragma: no cover
